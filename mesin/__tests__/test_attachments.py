...
def test_simpan_dan_daftar_lampiran(db):
    with database.buka(db) as kon:
        sid = database.tambah_siswa(kon, "AnakLamp", pemilik="guru")
        sesi_id = database.buat_sesi(kon, sid, seed=7)
        lid = database.simpan_lampiran(
            kon, sesi_id, "lembar-1.jpg", mime="image/jpeg",
            hasil_json='{"soal": []}',
        )
        daftar = database.daftar_lampiran(kon, sesi_id)
        satu = database.ambil_lampiran(kon, lid)
    assert len(daftar) == 1
...
def test_tandai_lampiran_diterapkan(db):
    with database.buka(db) as kon:
        sid = database.tambah_siswa(kon, "AnakLamp2")
        sesi_id = database.buat_sesi(kon, sid, seed=7)
        lid = database.simpan_lampiran(kon, sesi_id, "lembar-1.jpg")
...
def test_hapus_sesi_membersihkan_lampiran(db):
    ...
    with database.buka(db) as kon:
        sid = database.tambah_siswa(kon, "AnakLamp3")
        sesi_id = database.buat_sesi(kon, sid, seed=7)
        database.simpan_lampiran(kon, sesi_id, "lembar-1.jpg")
...
def test_http_upload_lampiran_mulai_ekstraksi(server, monkeypatch):
    ...
    with server.buka() as kon:
        sid = database.tambah_siswa(kon, "AnakLamp", pemilik="guru")
        sesi_id = database.buat_sesi(kon, sid, seed=7, mode="drill")
        jumlah_soal = len(database.isi_sesi(kon, sesi_id))
...
def test_http_upload_bukan_gambar_ditolak(server):
    ...
    assert kode == 400
    with server.buka() as kon:
        assert len(database.daftar_lampiran(kon, sesi_id)) == 0
...
def test_halaman_konfirmasi_menampilkan_foto_dan_usulan(db):
    with database.buka(db) as kon:
        sid = database.tambah_siswa(kon, "AnakKonf")
        sesi_id = database.buat_sesi(kon, sid, seed=7)
...
def test_halaman_konfirmasi_lampiran_tanpa_hasil(db):
    ...
    with database.buka(db) as kon:
        sid = database.tambah_siswa(kon, "AnakKonf2")
        sesi_id = database.buat_sesi(kon, sid, seed=7)
        lid = database.simpan_lampiran(kon, sesi_id, "lembar-1.jpg", hasil_json="")
        html = attachments.halaman_konfirmasi(kon, lid).decode()
    assert 'name="jwb_' in html
...
def test_halaman_konfirmasi_menampilkan_jawaban_lama_anak(db):
    ...
    with database.buka(db) as kon:
        sid = database.tambah_siswa(kon, "AnakDobel")
        sesi_id = database.buat_sesi(kon, sid, seed=7)
        ssid = database.isi_sesi(kon, sesi_id)[0]["sesi_soal_id"]
        database.simpan_jawaban(kon, ssid, jawaban="7", cara="hitung jari")
        lid = database.simpan_lampiran(kon, sesi_id, "lembar-1.jpg", hasil_json="")
        html = attachments.halaman_konfirmasi(kon, lid).decode()
...
def test_http_terapkan_lewat_form(server):
    ...
    with server.buka() as kon:
        sid = database.tambah_siswa(kon, "feby", pemilik="guru")
        sesi_id = database.buat_sesi(kon, sid, seed=7, mode="drill")
        lid = database.simpan_lampiran(kon, sesi_id, "lembar-1.jpg")
        kunci = database.isi_sesi(kon, sesi_id)[0]["kunci"]
        ssid = database.isi_sesi(kon, sesi_id)[0]["sesi_soal_id"]
