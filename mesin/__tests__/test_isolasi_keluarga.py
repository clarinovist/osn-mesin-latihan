...
def dua_keluarga(tmp_path, monkeypatch):
    ...
    with s.buka() as kon:
        ...
        b = basis.tambah_siswa(kon, "RaraB", "P4", pemilik="ortu-b")
        feby = basis.tambah_siswa(kon, "feby", "P3", pemilik="ortu-a")
        sesi_a = basis.buat_sesi(kon, a, seed=11)
        sesi_b = basis.buat_sesi(kon, b, seed=22)
        sesi_feby = basis.buat_sesi(kon, feby, seed=33)
        lamp_b = basis.simpan_lampiran(kon, sesi_b, "foto-b.jpg")
    sandi.tambah_akun("ortu-a", SANDI_A, "guru", path=sandi.BERKAS_SANDI)
    sandi.tambah_akun("ortu-b", SANDI_B, "guru", path=sandi.BERKAS_SANDI)
