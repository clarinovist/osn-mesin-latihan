...
bukan di murid.py. Test palang di test_murid.py tetap mengawasi.
"""
...
def test_jawaban_benar_dari_murid_langsung_didiagnosis(db):
    with basis.buka(db) as kon:
        sid = basis.tambah_siswa(kon, "AnakOtomatis")
...
def test_jawaban_malrule_dari_murid_langsung_dapat_kode(db):
    with basis.buka(db) as kon:
        sid = basis.tambah_siswa(kon, "AnakMalrule")
...
def test_mengaku_menebak_dari_murid_jadi_N_tanpa_guru(db):
    with basis.buka(db) as kon:
        sid = basis.tambah_siswa(kon, "AnakTebak")
...
def test_kode_manual_guru_bertahan_saat_murid_perbarui_jawaban(db):
    ...
    with basis.buka(db) as kon:
        sid = basis.tambah_siswa(kon, "AnakTimpa")
        ...
        assert basis.isi_sesi(kon, sesi_id)[0]["manual"] == 1
...
def test_halaman_murid_tetap_bersih_setelah_diagnosis_otomatis(db):
    ...
    with basis.buka(db) as kon:
        ...
        html = murid.halaman_kerja(kon, sid, sesi_id)
