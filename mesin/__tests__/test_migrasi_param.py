...
from migrasi_param import KLAUSUL, jalankan  # noqa: E402
...
def _baris_klausul(db, pasangan_level_seed):
    ...
    with basis.buka(db) as kon:
        sid = basis.tambah_siswa(kon, "Migrasi", "P3")
        for level, seed in pasangan_level_seed:
            sesi_id = basis.buat_sesi(kon, sid, seed=seed, level=level)
            hasil.extend(
                b
...
def test_soal_baru_terrestorasi_identik_tanpa_cabang(db):
    ...
    with basis.buka(db) as kon:
        sid = basis.tambah_siswa(kon, "Restorasi")
        sesi_id = basis.buat_sesi(kon, sid, seed=77)
        asli = buat_lembar(77).soal
        ulang = [
            web._soal_dari_baris(b) for b in basis.isi_sesi(kon, sesi_id)
        ]
