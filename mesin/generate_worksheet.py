...
def main() -> int:
    ...
    with database.buka() as kon:
        siswa = database.daftar_siswa(kon)
        if arg.siswa:
        ...
        for s in siswa:
            ...
            sesi_id = database.buat_sesi(kon, s["id"], seed)
            lembar = buat_lembar(seed)

            dasar = f"{hari_ini}-{s['nama'].lower()}-sesi{sesi_id}"
            f_soal = worksheets.tulis(
                KELUARAN / f"{dasar}-SOAL.html",
                worksheets.lembar_soal(list(lembar.soal), s["nama"], hari_ini),
            )
            f_nilai = worksheets.tulis(
                KELUARAN / f"{dasar}-PENILAIAN.html",
                worksheets.lembar_penilaian(
